...
class HorarioDisponible(models.Model):
    ...
    CONTINUO: El cliente puede elegir cualquier horario dentro del rango (ej: 9:00-18:00, cliente elige 10:30-12:00)
    """
    DIAS_SEMANA = [
        (0, 'Lunes'),
        (1, 'Martes'),
    ...
    duracion_minima_minutos = models.PositiveIntegerField(
        default=60,
        help_text='Duración mínima en minutos (solo aplica a slots CONTINUOS)',
...
class Reserva(models.Model):
    ...
    def clean(self):
        ...
        for horario in horarios_disponibles:
            if horario.tipo == 'DISCRETO':
                # Para slots DISCRETOS: debe coincidir exactamente
                if (self.hora_inicio == horario.hora_inicio and 
                    self.hora_fin == horario.hora_fin):
                    slot_valido = True
                    break
            
            elif horario.tipo == 'CONTINUO':
                # Para slots CONTINUOS: debe estar dentro del rango
                if (self.hora_inicio >= horario.hora_inicio and 
                    self.hora_fin <= horario.hora_fin):
        ...
        # Validar solapes solo con reservas CONFIRMADAS
        if self.estado in ['CONFIRMADA', 'PENDIENTE_CONFIRMACION'] or not self.pk:
            ...
            for reserva in solapadas:
                if (self.hora_inicio < reserva.hora_fin and 
                    ...
                    raise ValidationError(
                        f'Ya existe una reserva confirmada que solapa: '
    ...
    def enviar_email_verificacion(self, request=None):
        ...
        if request:
            url_completa = request.build_absolute_uri(url_verificacion)
        else:
            url_completa = url_verificacion
